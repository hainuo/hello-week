...
export class HelloWeek {
    ...
    private handleClickInteraction(target: HTMLElement, callback: CallbackFunction): void {
        target.addEventListener('click', (event: any) => {
            let currentTarget;
            if (target.className.indexOf('day_')) {
                currentTarget = event.target.parentElement;
            } else {
                currentTarget = event.target;
            }
            const index = Utilities.getIndexForEventTarget(this.daysOfMonth, currentTarget);
            // console.log(this.days[index]);
            if (!this.days[index] || this.days[index].locked) {
                return;
            }
            ...
            Utilities.toggleClass(currentTarget, HelloWeek.cssStates.IS_SELECTED);
            ...
            if (this.options.range) {
                ...
                if (this.intervalRange.begin && !this.intervalRange.end) {
                    ...
                    Utilities.addClass(currentTarget, HelloWeek.cssStates.IS_END_RANGE);
                    if (this.intervalRange.begin > this.intervalRange.end) {
                        this.intervalRange.begin = undefined;
                        this.intervalRange.end = undefined;
                        this.removeStatesClass();
                ...
                Utilities.addClass(currentTarget, HelloWeek.cssStates.IS_SELECTED);
            }
    ...
    private createDay(date: Date): void {
        ...
        const dayText = <any>document.createElement('div');
        const dayOtherText = <any>document.createElement('div');
        const dayOptions = {
            day: num,
        ...
        this.days = this.days || {};
        Utilities.addClass(dayText, HelloWeek.cssClasses.DAY + '_text');
        Utilities.addClass(dayText, (date.getMonth() + 1) + '_' + num);
        dayText.textContent = dayOptions.day;
        Utilities.addClass(dayOtherText, HelloWeek.cssClasses.DAY + '_other_text');
        Utilities.addClass(dayOtherText, (date.getMonth() + 1) + '_' + num);
        dayOtherText.textContent = ' ';

        newDay.appendChild(dayText);
        newDay.appendChild(dayOtherText);
        // console.log('34342343',newDay);
        // newDay.textContent ='<div class="other-text"><span>'+ dayOptions.day+'</span><span></span></div>';
        Utilities.addClass(newDay, HelloWeek.cssClasses.DAY);
        Utilities.addClass(newDay, 'zhou_' + day);
